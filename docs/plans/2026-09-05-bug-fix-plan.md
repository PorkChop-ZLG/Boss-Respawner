...
**状态：** 已执行（代码审查修复已完成；P3-2/P3-7 暂缓，跨存档 bug 待继续诊断）
...
| 任务 | 对应审查项 | 状态 |
|---|---|---|
| T1 | P1-1 重复笼检查性能优化 | ✅ 已完成 |
| T2 | P1-2 删除每区块数量限制 | ✅ 已完成 |
| T3 | P1-3 收敛为单一 `@Mod` 入口 | ✅ 已完成 |
| T4 | P2-1 `activation.consume` 接入 | ✅ 已完成 |
| T5 | P2-2 删除 `visual.*` | ✅ 已完成 |
| T6 | P2-3 客户端注册表解析缓存 | ✅ 已完成 |
| T7 | P2-4 `spawn.count > 1` 原子性 | ✅ 已完成 |
| T8 | P2-5 reload 校验 `activation.item` | ✅ 已完成 |
| T9 | P2-6 NBT 防御性校验 | ⚠️ 基本完成（`maxAttempts` 负值未显式钳制） |
| T10 | P2-7 `maxAttempts` 终态与警告 | ✅ 已完成 |
| T11 | P2-8 放置后同步客户端 | ✅ 已完成 |
| T12 | P2-9 删除 HomePosLocator，仅死亡点 | ✅ 已完成 |
| T13 | P3-1 `schema_version` 校验 | ✅ 已完成 |
| T14 | P3-6 未点亮时减少 tick 开销 | ✅ 已完成（保留 ticker，未点亮直接 return） |
| T15 | P3-8 reload 预解析 `spawn.nbt` | ✅ 已完成 |
| T16 | P3-9 渲染兜底非法注册对象 | ✅ 已完成 |
| T17 | P3-10 右键激活触发 `GameEvent.BLOCK_CHANGE` | ✅ 已完成 |
| T18 | P3-11 清理客户端调试日志 | ✅ 已完成 |
